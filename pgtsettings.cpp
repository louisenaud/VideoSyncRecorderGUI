...
PGTsettings::PGTsettings(QWidget *parent)
	: QDialog(parent)
{
	//QObject::connect(ui.okButton, SIGNAL(clicked()), this, SLOT(checkValues()));
	ui.setupUi(this);
}
...
void PGTsettings::on_applyButton_clicked() {
	//fps value
	QString fps_txt;
	...
	//check to make sure ROI values are valid
		...
		x_num < 40 || x_num >= 520 || y_num < 60 || y_num >= 360 ||
